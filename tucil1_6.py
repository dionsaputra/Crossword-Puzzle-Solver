...
def read_file(fileName):
	ttsFile = open(fileName,"r")
	ttsSize = int(ttsFile.readline())

	tts = [[0 for x in range(ttsSize)] for y in range(ttsSize)]
	for i in range(ttsSize):
		curRow = ttsFile.readline()
		for j in range(ttsSize):
			tts[i][j] = curRow[j]

	words = ttsFile.readline().split(';')
	...
	if length == len(string):
		...
		if datar:
			while i<length and match:
				if tts[head[0]][head[1]+i] == c1 or tts[head[0]][head[1]+i] == string[i]:
					i += 1
				else:
					match = False
		else:
			while i<length and match:
				if tts[head[0]+i][head[1]] == c1 or tts[head[0]+i][head[1]] == string[i]:
					i += 1
				else:
					match = False
		return match
...
def permutasi_solusi(solusi,length):
	...
	if (pos1 > 0):
		...
		while solusi[length][pos2][0] < solusi[length][pos1-1][0]:
			pos2 -= 1
		[solusi[length][pos1-1],solusi[length][pos2]] = [solusi[length][pos2],solusi[length][pos1-1]]
...
while not is_solved(top(list_sol)):
	...
	while i < len(curblocks) and matchAll:
		idxsol = get_idx_sol(newtts,newsol,level,curblocks[i][0],curblocks[i][1],curblocks[i][2])
		if idxsol != -1:
			fill_tts(newtts,curblocks[i][0],curblocks[i][1],curblocks[i][2],newsol[level][idxsol][1])
